...
def custom_executable(node, something):
    ...
    if execution_tag == "execute_program":
        try:
            current_component_xunit_testcase = XUnitTestCase(
                component_name,
                "",
                test_type="failure")
            ...
            a_software_component = extract_software_component(
                component_name,
            ...
            if len(process_executor.process_observers) > 0:
                all_program_observers.append(
                    process_executor.process_observers)
            if len(process_executor.blocking_process_observers) > 0:
                all_program_observers.append(
                    process_executor.blocking_process_observers)
            if len(process_executor.blocking_process_observers) == 0:
            ...
            current_component_xunit_testcase.contents = str(
                sys.exc_traceback.tb_lineno) + str(e)  # @UndefinedVariable

            log.error("Error while executing a user defined component. %s",
                      str(e))
            ...
            current_component_xunit_testcase.contents = str(
                sys.exc_traceback.tb_lineno) + str(e)  # @UndefinedVariable
            # TODO: Catch KeyError (occurred with missing '<' in scxml)
            log.error("A wild error occurred while executing component '%s'",
                      component_name)
            log.debug("Error in line %s! Message: %s",
                      (sys.exc_traceback.tb_lineno), e)  # @UndefinedVariable
            log.debug("Traceback: %s", (traceback.format_exc()))
            state_machine.send("execute_program.fail")

    elif execution_tag == "emergency_exit":
        current_component_xunit_testcase = XUnitTestCase(
            "DEPRECATED_Emergency_Exit",
            component_name,
            test_type="failure")
        ...
        current_component_xunit_testcase = XUnitTestCase("ERROR",
                                                         component_name,
                                                         test_type="failure")
        ...
        log.debug("<Next State> and <Current State>: %s",
                  state_machine.interpreter.configuration)
        ...
        try:
            ...
            log.debug(">> We have %d observers configured, shutdown now",
                      id_obs)
            id_obs = 0
            obs_idx = 0
            for observer_list in all_program_observers:
                for observer in observer_list:
                    obs_idx += 1
                    log.debug("Observer #%d status: %s %s", obs_idx,
            ...
            log.debug("Shutdown %d observers, the rest already exited " +
                      "(NOT ONGOING)", id_obs)
        except Exception as e:
                log.warning("Unknown Exception occurred during observer " +
                            "'war': %s", e)
        ...
        all_program_executors_sorted = sorted(all_program_executors.itervalues(),
                                              reverse=True)
        ...
        if negative_result:
            ...
            write_fsm_file("negative_result",
                           trial_path + "/fsmstatus",
                           "ERROR")
            ...
            log.debug("blockdiag written to %s",
                      trial_path + "/blockdiag.diag")
        ...
        log.critical('"Custom Executable" called but the tag ' +
                     '"%s" was not recognised!', execution_tag)

    if current_component_xunit_testcase:
        state_machine.xunit_xml_builder.add_test_case(
            current_component_xunit_testcase)
