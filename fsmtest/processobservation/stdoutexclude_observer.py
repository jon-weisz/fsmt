...
class StdoutexcludeObserver(ProcessObserver):
	...
	def run(self):
		...
		if self.check_type.criteria == "":
			...
			self.log_success()
			self.log.info("Exiting STDOUTEXCLUDE observer for %s, no criteria given", component_name)
		...
		while ((time.time() - t0) < self.check_type.timeout) and not self.end_thread:
			...
			if log_lines != "" and log_lines != "\n" and been_read != 0:
				...
				if "IOERROR" in log_lines:
					self.log.error("IO Error in component %s output", component_name)
		...
		if self.hit_timeout:
			if (time.time() - t0) < self.check_type.timeout:
				...
				self.log_success("hit timeout (%ds) while looking for '%s'. Good!" % (self.check_type.timeout, criteria))
				self.send_positive_result_to_pyscxml()
				self.end_thread = True
