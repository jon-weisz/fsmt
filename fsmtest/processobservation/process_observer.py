...
class ProcessObserver(threading.Thread):
    ...
    def __init__(self, process_executer_, parent_pipe_, check_type_, process_exchange_data_, state_machine_):
        """
    ...
    def log_start(self):
        ...
        try:
            ...
            self.log.info("%s [%s] %s observer is starting [block = %s]",
                          self.process_executer.software_component.name,
                          str(self.process_executer.subprocess_pid),
                          str(self.check_type.type).upper(),
                          self.check_type.blocking)
        except IOError as e:
            self.log.error("Emergency exit %s" % e)
            self.state_machine.unsatisfied = True
            self.state_machine.send("unsatisfied_criteria")
    ...
    def log_success(self, message=""):
        ...
        """
        try:
            self.log.info("%s [%s] %s observer %s",
                          self.process_executer.software_component.name,
                          str(self.process_executer.subprocess_pid),
    ...
    def get_my_name(self):
        ...
        :return:
        """
        return "%s:%s" % (self.process_executer.software_component.name, str(self.process_executer.subprocess_pid))
    ...
    def run(self):
        ...
        :raise:
        """
        raise NotImplementedError("You should have implemented the run() method")
