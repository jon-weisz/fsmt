...
class PidObserver(ProcessObserver):
    ...
    def __init__(self, process_executer, parent_pipe, check_type,
        ...
        """
        ProcessObserver.__init__(self, process_executer, parent_pipe,
                                 check_type, process_exchange_data,
                                 state_machine_)

    def run(self):
        the_pid = self.process_executer.subprocess_pid
        comp_name = self.process_executer.software_component.name
        self.log_start()

        t0 = time.time()
        while (((time.time() - t0) < self.check_type.timeout)
               and not self.end_thread):
            ...
            if not hasattr(self.proc_exec_subprocess, "pid"):
                ...
                self.log_failure(
                    "No PID found on initialisation. " +
                    " _NOT_ starting observation")
            ...
            if sys.platform == 'darwin':
                ...
                if not os.path.exists("/proc/%s" % the_pid):
                    ...
                    break
        # At this point we either came here because of a "break", or the time
        # is up.
        if self.internalOK:
            self.send_positive_result_to_pyscxml()
            t = time.time()
            self.log_success("PID was found in %sms" %
                             str(((t - t0) * 1000))[:4])
        else:
            t = time.time()
            self.log_failure(
