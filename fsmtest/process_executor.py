    ...
    def component_runner_function(self):
        ...
        if self.software_component.host == "localhost":
            ...
            for an_observer in itertools.chain(self.process_observers, self.blocking_process_observers):
                ...
                self.log.debug("(%s) %s Observer starts now", an_observer.check_type.type, self.software_component.name)
                an_observer.start()

            # DO NOT CHANGE THIS!
            sub_proc_ret_code = self.subprocess.wait()
            self.log.debug((self.software_component.name, len(self.process_observers) + len(self.blocking_process_observers)))
            ...
            self.log.info("Distributed execution is in preparation please do not use any other hostname than localhost")
            '''
