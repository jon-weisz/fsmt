...
class ColorLog(object):
    ...
    colormap = dict(
        ...
        critical=dict(color='red', attrs=['bold', 'underline']),  # TODO: Check why outline is not working
    ...
    def __init__(self, logger):
        ...
        def find_caller_no_lambda():
            ...
            while hasattr(f, "f_code"):
                ...
                if filename == logging._srcfile or filename.find(
                        'fsmtest/Utils.py') > 0 or f.f_code.co_name == "<lambda>":
    ...
    def __getattr__(self, name):
        ...
        :return:
        """
        if name in ['debug', 'info', 'warn', 'warning', 'error', 'critical', 'outline']:
    ...
    def _setup_custom_logger(self, use_colour_logging,
        ...
        class SingleLevelFilter(logging.Filter):
            ...
            def filter(self, record):
                ...
                """
                if self.reject:
                    return (record.levelno > self.passlevel) and (record.levelno != self.stream_level)
                else:
                    return (record.levelno <= self.passlevel) or (record.levelno == self.stream_level)
        ...
        format_args_part_a = dict(color='white', attrs=['bold'])
        format_args_part_b = dict(color='white', attrs=['bold', 'underline'])
        ...
        if log_to_console:
            ...
            STDOUT_console_filter = SingleLevelFilter(logging.INFO, logging.STREAM, False)
            ...
            STDERRconsoleFilter = SingleLevelFilter(logging.INFO, logging.STREAM, True)
        ...
        if log_to_file:
            ...
            filename = os.path.join(logging_directory, "FSMT-" + username + "-" + str(curTime) + "_full.log")
